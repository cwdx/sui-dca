...
export function isTreasury(type: string): boolean {
  ...
  return (
    type ===
    `${getTypeOrigin("flowx-amm", "treasury::Treasury")}::treasury::Treasury`
  );
}
...
export class Treasury implements StructClass {
  ...
  static readonly $typeName: `${string}::treasury::Treasury` = `${getTypeOrigin(
    "flowx-amm",
    "treasury::Treasury",
  ...
  static reified(): TreasuryReified {
    ...
    return {
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Treasury.fromFieldsWithTypes(item),
      fromBcs: (data: Uint8Array) =>
        Treasury.fromFields(reifiedBcs.parse(data)),
      bcs: reifiedBcs,
      fromJSONField: (field: any) => Treasury.fromJSONField(field),
      fromJSON: (json: Record<string, any>) => Treasury.fromJSON(json),
      fromSuiParsedData: (content: SuiParsedData) =>
        Treasury.fromSuiParsedData(content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Treasury.fromSuiObjectData(content),
      fetch: async (client: SupportedSuiClient, id: string) =>
        Treasury.fetch(client, id),
  ...
  private static cachedBcs: ReturnType<typeof Treasury.instantiateBcs> | null =
    null;
  ...
  static fromSuiParsedData(content: SuiParsedData): Treasury {
    ...
    if (!isTreasury(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Treasury object`,
      );
    }
