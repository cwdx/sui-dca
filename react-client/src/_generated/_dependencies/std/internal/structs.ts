...
export interface PermitFields<_T extends PhantomTypeArgument> {
  dummyField: ToField<"bool">;
}

export type PermitReified<T extends PhantomTypeArgument> = Reified<
  Permit<T>,
  PermitFields<T>
>;

export type PermitJSONField<_T extends PhantomTypeArgument> = {
  dummyField: boolean;
};
...
export class Permit<T extends PhantomTypeArgument> implements StructClass {
  ...
  static reified<T extends PhantomReified<PhantomTypeArgument>>(
    ...
    return {
      ...
      fromFieldsWithTypes: (item: FieldsWithTypes) =>
        Permit.fromFieldsWithTypes(T, item),
      fromBcs: (data: Uint8Array) =>
        Permit.fromFields(T, reifiedBcs.parse(data)),
      bcs: reifiedBcs,
      fromJSONField: (field: any) => Permit.fromJSONField(T, field),
      fromJSON: (json: Record<string, any>) => Permit.fromJSON(T, json),
      fromSuiParsedData: (content: SuiParsedData) =>
        Permit.fromSuiParsedData(T, content),
      fromSuiObjectData: (content: SuiObjectData) =>
        Permit.fromSuiObjectData(T, content),
      fetch: async (client: SupportedSuiClient, id: string) =>
        Permit.fetch(client, T, id),
  ...
  private static cachedBcs: ReturnType<typeof Permit.instantiateBcs> | null =
    null;
  ...
  static fromSuiParsedData<T extends PhantomReified<PhantomTypeArgument>>(
    ...
    if (!isPermit(content.type)) {
      throw new Error(
        `object at ${(content.fields as any).id} is not a Permit object`,
      );
    }
