...
function isGrpcClient(client: unknown): client is SuiGrpcClient {
  return (
    client != null && typeof client === "object" && "ledgerService" in client
  );
}
...
async function fetchObjectBcsWithGrpc(
  ...
  try {
    ...
    if (!contents) {
      throw new Error(
        `error fetching object at id ${id}: object not found or not a Move object`,
      );
    }
    if (!contents.value || !contents.name) {
      throw new Error(
        `error fetching object at id ${id}: no BCS data returned`,
      );
    }
...
async function fetchObjectBcsWithGraphQL(
  ...
  if (res.errors && res.errors.length > 0) {
    throw new Error(
      `error fetching object at id ${id}: ${JSON.stringify(res.errors)}`,
    );
  }
