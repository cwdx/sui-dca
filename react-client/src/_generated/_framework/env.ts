...
export function setActiveEnv(
  ...
  if (!envRegistry[name]) {
    const available =
      Object.keys(envRegistry).join(", ") || "(none registered)";
...
export function getActiveEnv(): EnvConfig {
  if (!activeEnv) {
    throw new Error(
      "No active environment. Call setActiveEnv() or import the envs module first.",
    );
  }
  return activeEnv;
}
...
export function getPackageConfig(pkgName: string): PackageConfig {
  ...
  if (!config) {
    throw new Error(
      `Package '${pkgName}' not found in active environment '${activeEnvName}'`,
    );
  }
  return config;
}
...
export function getDependencyConfig(pkgName: string): PackageConfig {
  ...
  if (!config) {
    throw new Error(
      `Dependency '${pkgName}' not found in active environment '${activeEnvName}'`,
    );
  }
  return config;
}
