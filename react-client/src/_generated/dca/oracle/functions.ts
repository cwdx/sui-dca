 ...
 */
export function multiplyPrices(
  tx: Transaction,
  args: MultiplyPricesArgs,
): TransactionResult {
  return tx.moveCall({
    target: `${getPublishedAt("dca")}::oracle::multiply_prices`,
    arguments: [obj(tx, args.price1), obj(tx, args.price2)],
  });
}
 ...
 */
export function addExponents(
  tx: Transaction,
  args: AddExponentsArgs,
): TransactionResult {
  return tx.moveCall({
...
export function pow10(
  tx: Transaction,
  exp: bigint | TransactionArgument,
): TransactionResult {
  return tx.moveCall({
