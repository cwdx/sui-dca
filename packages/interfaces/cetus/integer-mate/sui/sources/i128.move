module integer_mate::i128 {
    ...
    public fun overflowing_sub(num1: I128, num2: I128): (I128, bool) {
        ...
        let overflow =
            (sign(num1) & sign(sub_num) & u8_neg(sign(sum))) + (u8_neg(sign(num1)) & u8_neg(sign(sub_num)) & sign(sum));
        (sum, overflow != 0)
    }
    ...
    fun test_add() {
        ...
        assert!(
            as_u128(add(from(10000), neg_from(99999))) == 0xfffffffffffffffffffffffffffea071,
            2,
        );
        assert!(as_u128(add(from(99999), neg_from(10000))) == 89999, 2);
        assert!(
            as_u128(add(neg_from(MIN_AS_U128), from(1))) == 0x80000000000000000000000000000001,
            2,
        );
    ...
    fun test_wrapping_sub() {
        ...
        assert!(
            as_u128(wrapping_sub(neg_from(1000000), neg_from(1))) == as_u128(neg_from(999999)),
            0,
        );
        ...
        assert!(
            as_u128(wrapping_sub(from(MAX_AS_U128), from(1))) == as_u128(from(MAX_AS_U128 - 1)),
            0,
        );
        assert!(
        ...
        assert!(
            as_u128(wrapping_sub(neg_from(MIN_AS_U128), from(1))) == as_u128(from(MAX_AS_U128)),
            0,
        );
    }
    ...
    fun test_mul() {
        ...
        assert!(
            as_u128(mul(from(MIN_AS_U128/2), neg_from(2))) == as_u128(neg_from(MIN_AS_U128)),
            0,
        );
    }
