module move_stl::linked_table {
    use std::option::{Self, Option, none, is_none, is_some, swap_or_fill, some};
    ...
    public fun head<K: store + drop + copy, V: store>(
        table: &LinkedTable<K, V>,
    ): option::Option<K> {
        abort (0)
    }

    public fun tail<K: store + drop + copy, V: store>(
        table: &LinkedTable<K, V>,
    ): option::Option<K> {
        abort (0)
    }
    ...
    public fun borrow_mut<K: store + drop + copy, V: store>(
    ...
    public fun borrow_node<K: store + drop + copy, V: store>(
        table: &LinkedTable<K, V>,
        key: K,
    ): &Node<K, V> {
        abort (0)
    ...
    public fun borrow_mut_node<K: store + drop + copy, V: store>(
        table: &mut LinkedTable<K, V>,
        key: K,
    ): &mut Node<K, V> {
        abort (0)
    ...
    public fun push_back<K: store + drop + copy, V: store>(
        table: &mut LinkedTable<K, V>,
        key: K,
        value: V,
    ) {
    ...
    public fun push_front<K: store + drop + copy, V: store>(
        table: &mut LinkedTable<K, V>,
        key: K,
        value: V,
    ) {
    ...
    public fun insert_before<K: store + drop + copy, V: store>(
        table: &mut LinkedTable<K, V>,
        current_key: K,
        key: K,
        value: V,
    ...
    public fun insert_after<K: store + drop + copy, V: store>(
        table: &mut LinkedTable<K, V>,
        current_key: K,
        key: K,
        value: V,
