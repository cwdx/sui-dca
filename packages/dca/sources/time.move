module dca::time {
    ...
    // Check if `n` seconds have passed from `a_ts` to `b_ts`
    // Uses saturating subtraction to prevent underflow if mean_deviation > n
    fun has_n_seconds_passed_(b_ts: u64, a_ts: u64, n: u64, mean_deviation: u64): bool {
        assert!(b_ts >= a_ts, 0);
        let time_passed = b_ts - a_ts;
        // Saturating subtraction: if mean_deviation >= n, threshold is 0 (immediate execution allowed)
        let threshold = if (mean_deviation >= n) { 0 } else { n - mean_deviation };
        time_passed >= threshold
    }
