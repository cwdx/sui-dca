...
export function isResult(type: string): boolean {
  ...
  return type === `${getTypeOrigin('flowx-amm', 'comparator::Result')}::comparator::Result`
}
...
export class Result implements StructClass {
  ...
  static readonly $typeName: `${string}::comparator::Result` = `${
    getTypeOrigin('flowx-amm', 'comparator::Result')
  ...
  static reified(): ResultReified {
    ...
    return {
      ...
      fetch: async (client: SupportedSuiClient, id: string) => Result.fetch(client, id),
  ...
  private static cachedBcs: ReturnType<typeof Result.instantiateBcs> | null = null
